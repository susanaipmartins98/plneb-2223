...
while re.search(r"#E=.+\n#E=.+", collapse_explications) != None:
    collapse_explications = re.sub(r"(#E=.+)\n#E=(.+)", r"\1 \2", collapse_explications)
remove_marks = re.sub(r"#[TE]=", r"", collapse_explications)
entries = re.findall(r"\n\n(.+)\n(.+)", remove_marks)

newfile1 = open("1_no_term_ff.txt", "w", encoding="utf-8")
newfile1.write(remove_term_ff)
newfile2 = open("2_no_explication_ff.txt", "w", encoding="utf-8")
newfile2.write(remove_explication_ff)
newfile3 = open("3_final.txt", "w", encoding="utf-8")
